...
class TestDependencyValidationIntegration(unittest.TestCase):
    ...
    def test_validation_auto_fixes_invalid_plan(self, mock_ai, mock_get_diff, mock_subprocess, mock_progress):
        """Test that validation automatically fixes invalid plans."""
        ...
        with patch('git_smart_squash.cli.parse_diff', return_value=hunks):
            ...
            # The run_smart_squash method should exit early due to validation failure
            try:
            ...
            # Verify automatic fixing message was printed instead of error
            fixing_calls = [call for call in self.cli.console.print.call_args_list 
                          if "Dependency violations detected" in str(call)]
            self.assertTrue(len(fixing_calls) > 0, "Expected automatic fixing message")
            ...
            self.assertEqual(len(error_calls), 0, "Should not show old error message")
            
            # Verify success message was shown
            success_calls = [call for call in self.cli.console.print.call_args_list 
                           if "Successfully reorganized" in str(call)]
            self.assertTrue(len(success_calls) > 0, "Expected success message after auto-fix")
