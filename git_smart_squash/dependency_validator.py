...
class DependencyValidator:
    ...
    def suggest_fixes(self, validation_result: ValidationResult, commits: List[dict]) -> List[str]:
        """Suggest fixes for validation errors."""
        suggestions = []
        
        # Extract commit indices that need to be merged from error messages
        commits_to_merge = set()
        for error in validation_result.errors:
            # Parse error messages to find commits that need merging
            if "depends on" in error and "commit #" in error:
                # Extract commit numbers from error message
                import re
                matches = re.findall(r'commit #(\d+)', error)
                if len(matches) >= 2:
                    commits_to_merge.update(int(m) - 1 for m in matches)
        
        if commits_to_merge:
            commit_nums = sorted(commits_to_merge)
            suggestions.append(
                f"Consider merging commits {', '.join(f'#{n+1}' for n in commit_nums)} "
                f"to resolve dependency conflicts"
            )
